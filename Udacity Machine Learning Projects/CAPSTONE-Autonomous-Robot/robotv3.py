...
# This implementation searches for every square in the grid before stopping and proceeding to the second run
import numpy as np
...
class Robot(object):
    def __init__(self, maze_dim, number):
        ...
        # List of cells that we still need to explore to 
        self.goal_list = []
        self.dist_to_g = [[99 for i in range(self.dimensions)] for j in range(self.dimensions)]
        
        # Put the goals into a list
        for a in range(maze_dim):
            if a % 2 == 0:
                for b in range(maze_dim):
                    self.goal_list.append([a,b])
            else:
                for b in reversed(range(maze_dim)):
                    self.goal_list.append([a,b])
        ...
        self.location = [0, 0]
        self.heading = 'up'     
        #Keep track of which cells we have found               
    ...
    def next_move(self, sensors):
        ...
        if self.location == goal:
            ...
            except: self.found_this_goal = True
        ...
        elif self.found_this_goal == True and len(self.goal_list) == 0 and self.ready_to_reset == False:
            for a in range(self.dimensions):
                for b in range(self.dimensions):
                    if info_on[a][b] == 0:
                        # Keep track of the cells we know nothing about
        ...
        try: t = self.goal_list.index(self.location)
        except: pass
        
        # remove the current location from the goal_list if it is there
        try:
            self.goal_list.remove(self.location)
        except:
            pass
            
        # remvore the entry for found_goal corresponding to that location
        try:
            self.found_goal.pop(t)
        except:pass
