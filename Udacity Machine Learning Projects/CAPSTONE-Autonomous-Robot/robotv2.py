...
# This implementation looks for a combination of any number of the corners of the grid first and then looks for the center goal last goal
...
class Robot(object):
    def __init__(self, maze_dim, number):
        ...
        if number == 1:
            ...
            self.final_goal = [self.dimensions/2 - 1, self.dimensions/2]
            
        # Define the cells to explore to before the final goal
        self.top_left_corner = [0,self.dimensions - 1]
        self.top_right_corner = [self.dimensions - 1,self.dimensions - 1]
        self.bottom_right_corner = [self.dimensions - 1,0]
        
        # Define the cells to visit prior to the final goal cell
        self.goals = [self.top_right_corner, self.bottom_right_corner, self.top_left_corner]
        
        
        d = []
       
        # Select the number of cells to visit before the final cell
        a = 2
        d = list(itertools.permutations(self.goals,a))
        
        # Select a random permutation of these cells
        self.goal_list = random.choice(d)
        self.goal_list = list(self.goal_list)
        
        #or define a list of intermediate goals in another way
        # self.goal_list = ....
        
        # Append the final Goal to this list
        ...
        self.location = [0, 0]
        self.heading = 'up'                    
        self.found_goal = [False for a in range(len(self.goal_list))]
        self.found_this_goal = False
    ...
    def next_move(self, sensors):
        ...
        # Set the default goal as the center
        goal = self.final_goal
                
        # Set the goal for this round            
        if self.found_goal[0] == False:
            goal = self.goal_list[0]
        else:
            for a in range(1,len(self.goal_list)):
                if self.found_goal[a] == False and self.found_goal[a-1] == True:
                    goal = self.goal_list[a]
        
        # Set a flag indicating we have not found the goal for this round
        self.found_this_goal = False
        
        # Check to see if we have reached the goal for this round or not
        if self.location == goal:
            # If so, set the entry in self.found_goal corresponding to the goal to True
            x = self.goal_list.index(goal)
            self.found_goal[x] = True
            # Set the found_this_goal flag to True
            self.found_this_goal = True
       

        

        # If we haven't found the goal              
        if self.found_this_goal == False:
            # Perform the flood fill algorithm
            self.flood_algorithm(goal[0], goal[1])
        # If we have found the goal and the goal is the final goal and we are not ready to reset:
        elif self.found_this_goal == True and goal == self.final_goal and self.ready_to_reset == False:
            for a in range(self.dimensions):
                for b in range(self.dimensions):
                    if info_on[a][b] == 0:
                        # Keep track of the cells we know nothing about
        ...
        if self.first_run_complete == True:
            ...
            if len(potential_move_list) >0 and self.location != goal:
